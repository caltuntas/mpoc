...
export class SpecificationEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    let chars = this.http.get('http://localhost:8080/productspec/getCharacteristics');
    let prod = this.http.get('http://localhost:8080/productspec/getSpecForEdit/' + this.id);
    ...
    // this.http
    //   .get(`http://localhost:8080/productspec/getSpecForEdit/` + this.id)
    //   .subscribe(data => {
    //     this.productSpec =  <productSpecEditModel>data.json();
       
    //   });

    // this.productSpec.selectedCharacteristics.forEach(selectedChar => {
    //   this.newMethod(selectedChar);
    //   selectedChar.selectedValueIds.forEach(selectedValueId => {
    //     this.characteristics
    //       .find(x => x.id == selectedChar.id)
    //       .values.find(x => x.id == selectedValueId).isSelected = true;
